{% macro create_seed_executions_table_if_not_exists(schema_name, table_name) -%}
    ...
    {%- if adapter.get_relation(database=database, schema=schema_name, identifier=table_name) is none -%}
        ...
        {%- set query -%}
            {{ adapter.dispatch('get_create_seed_executions_table_if_not_exists_statement', 'dbt_artifacts')(schema_name, table_name) }}
        {% endset %}
...
{% macro databricks__get_create_seed_executions_table_if_not_exists_statement(schema_name, table_name) -%}
...
{% macro default__get_create_seed_executions_table_if_not_exists_statement(schema_name, table_name) -%}
