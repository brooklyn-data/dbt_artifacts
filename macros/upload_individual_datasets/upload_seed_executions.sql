    ...
    {% if seeds != [] %}
        ...
        {% for model in seeds -%}
            (
                ...
                {% set compile_started_at = (model.timing | selectattr("name", "eq", "compile") | first | default({}))["started_at"] %}
                {% if compile_started_at %}CAST('{{ compile_started_at }}' AS timestamp){% else %}CAST(null AS timestamp){% endif %}, {# compile_started_at #}
                {% set query_completed_at = (model.timing | selectattr("name", "eq", "execute") | first | default({}))["completed_at"] %}
                {% if query_completed_at %}CAST('{{ query_completed_at }}' AS timestamp){% else %}CAST(null AS timestamp){% endif %}, {# query_completed_at #}
