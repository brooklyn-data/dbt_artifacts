    ...
    {% if snapshots != [] %}
        ...
        {% for model in snapshots -%}
            (
                '{{ invocation_id }}', {# command_invocation_id #}
                '{{ model.node.unique_id }}', {# node_id #}
                cast('{{ run_started_at }}' as {{ type_timestamp() }}), {# run_started_at #}

                {% set config_full_refresh = model.node.config.full_refresh %}
                {% if config_full_refresh is none %}
                    {% set config_full_refresh = flags.FULL_REFRESH %}
                {% endif %}
                cast('{{ config_full_refresh }}' as {{ type_boolean() }}), {# was_full_refresh #}

                '{{ model.thread_id }}', {# thread_id #}
                '{{ model.status }}', {# status #}

                {% set compile_started_at = (model.timing | selectattr("name", "eq", "compile") | first | default({}))["started_at"] %}
                {% if compile_started_at %}cast('{{ compile_started_at }}' as {{ type_timestamp() }}){% else %}null{% endif %}, {# compile_started_at #}
                {% set query_completed_at = (model.timing | selectattr("name", "eq", "execute") | first | default({}))["completed_at"] %}
                {% if query_completed_at %}cast('{{ query_completed_at }}' as {{ type_timestamp() }}){% else %}null{% endif %}, {# query_completed_at #}
