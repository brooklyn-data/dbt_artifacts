...
{#-
    ...
    daily model; adds the target_name cut. Additive count columns sum to the
    daily model across targets; distinct_commands does not (a command may run
    under several targets) and is therefore per-target here.
-#}
...
with
    ...
    final as (
        select
            {{ dbt_artifacts.generate_surrogate_key(["inv.date_day", "inv.target_name"]) }}
                as run_health_daily_detail_id
            , inv.date_day
            , inv.target_name
            , inv.invocations
        ...
        left join node_daily as nd
            on inv.date_day = nd.date_day and inv.target_name = nd.target_name
        left join failed_invocation_daily as fid
            on inv.date_day = fid.date_day and inv.target_name = fid.target_name
    )
