...
anchor as (
    ...
    select
        ...
        coalesce(node_dependencies_deduped.depends_on_node_id, '')
            as depends_on_node_id,
        coalesce(latest_executions.total_node_runtime, 0)
            as total_node_runtime
...
search_path (node_ids, total_time) as (
    ...
    select
        ...
        array_cat(
                to_array(all_needed_dependencies.depends_on_node_id),
                search_path.node_ids)
            as node_ids,
